  ...
  .defines(function() {
    ...
    ig.Debug = ig.Class.extend({
      ...
      addPanel: function(panelDef) {
        ...
        if (panelDef.options) {
          for (var i = 0; i < panelDef.options.length; i++) {
            ...
            panel.addOption(
              new ig.DebugOption(opt.name, opt.object, opt.property),
            );
          }
        }
      ...
      togglePanel: function(panel) {
        ...
        panel.toggle(active);
        panel.menuItem.className =
          'ig_debug_menu_item' + (active ? ' active' : '');
      ...
      beforeRun: function() {
        ...
        this.debugTickAvg =
          this.debugTickAvg * 0.8 + (timeBeforeRun - this.debugRealTime) * 0.2;
