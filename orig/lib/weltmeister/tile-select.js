ig.module('weltmeister.tile-select').defines(function() {
  'use strict';
  ...
  wm.TileSelect = ig.Class.extend({
    ...
    setPosition: function(x, y) {
      ...
      this.pos.x = this.pos.x.limit(
        0,
        ig.system.width -
          this.layer.tiles.width -
          ig.system.width % this.layer.tilesize,
      );
      this.pos.y = this.pos.y.limit(
        0,
        ig.system.height -
          this.layer.tiles.height -
          ig.system.height % this.layer.tilesize,
      );
    },
      ...
      for (var ty = r.y; ty < r.y + r.h; ty++) {
        ...
        for (var tx = r.x; tx < r.x + r.w; tx++) {
          if (tx < 0 || ty < 0 || tx >= mw || ty >= mh) {
            ...
            row.push(
              ty * Math.floor(this.layer.tiles.width / this.layer.tilesize) +
                tx +
                1,
            );
          }
    ...
    getSelectionRect: function(x, y) {
      ...
      var txb = Math.floor((sx - this.pos.x) / this.layer.tilesize),
