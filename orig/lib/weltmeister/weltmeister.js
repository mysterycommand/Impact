...
ig
  .module('weltmeister.weltmeister')
  .requires(
    'dom.ready',
    'impact.game',
  ...
  .defines(function() {
    ...
    wm.Weltmeister = ig.Class.extend({
      ...
      init: function() {
        ...
        // Dialogs
        this.loadDialog = new wm.ModalDialogPathSelect(
          'Load Level',
          'Load',
          'scripts',
        );
        ...
        this.saveDialog = new wm.ModalDialogPathSelect(
          'Save Level',
          'Save',
          'scripts',
        );
        ...
        $('#levelSaveAs').bind(
          'click',
          this.saveDialog.open.bind(this.saveDialog),
        );
        ...
        this.tilesetSelectDialog = new wm.SelectFileDropdown(
          '#layerTileset',
        ...
        $('#buttonRemoveLayer').bind(
          'click',
          this.deleteLayerDialog.open.bind(this.deleteLayerDialog),
        );
        $('#buttonSaveLayerSettings').bind(
          'click',
          this.saveLayerSettings.bind(this),
        );
        $('#reloadImages').bind('click', ig.Image.reloadCache);
        $('#layerIsCollision').bind(
          'change',
          this.toggleCollisionLayer.bind(this),
        );
      ...
      uikeydown: function(event) {
        ...
        if (key.match(/^\d$/)) {
          ...
          var layer =
            name == 'entities' ? this.entities : this.getLayerWithName(name);
      ...
      showLoadDialog: function() {
        if (this.modified) {
          this.loseChangesDialog.onOk = this.loadDialog.open.bind(
            this.loadDialog,
          );
      ...
      setWindowTitle: function() {
        document.title =
          this.fileName + (this.modified ? ' * ' : ' - ') + 'Weltmeister';
      ...
      scroll: function(x, y) {
        ...
        this._rscreen.x =
          Math.round(this.screen.x * ig.system.scale) / ig.system.scale;
        this._rscreen.y =
          Math.round(this.screen.y * ig.system.scale) / ig.system.scale;
      ...
      touchScroll: function(event) {
        ...
        this.scroll(
          event.wheelDeltaX / ig.system.scale,
          event.wheelDeltaY / ig.system.scale,
        );
        this.draw();
        return false;
      },
      ...
      zoom: function(delta) {
        ...
        wm.config.view.zoom = z.limit(
          wm.config.view.zoomMin,
          wm.config.view.zoomMax,
        );
        wm.config.labels.step = Math.round(
          this.labelsStep / wm.config.view.zoom,
        );
        $('#zoomIndicator')
      ...
      save: function(dialog, path) {
        ...
        if (wm.config.project.outputFormat == 'module') {
          ...
          var levelName = levelModule.replace(/(^.*\.|-)(\w)/g, function(
            m,
            s,
            a,
          ) {
      ...
      addLayer: function() {
        ...
        newLayer.resize(
          wm.config.layerDefaults.width,
          wm.config.layerDefaults.height,
        );
      ...
      saveLayerSettings: function() {
        ...
        if (
          newWidth != this.activeLayer.width ||
          newHeight != this.activeLayer.height
        ) {
          ...
          if (this.mode == this.MODE.DEFAULT) {
            ...
            if (this.activeLayer == this.entities) {
              ...
              if (ig.input.state('select')) {
                this.activeLayer.beginSelecting(
                  ig.input.mouse.x,
                  ig.input.mouse.y,
                );
              } else {
            ...
            ig.input.state('select')
          ) {
            this.activeLayer.tileSelect.beginSelecting(
              ig.input.mouse.x,
              ig.input.mouse.y,
            );
          }
        ...
        if (action == 'delete') {
          ...
          if (
            ...
            if (this.activeLayer == this.entities) {
              ...
              this.mode = this.MODE.TILESELECT;
              this.activeLayer.tileSelect.setPosition(
                ig.input.mouse.x,
                ig.input.mouse.y,
              );
            }
      ...
      drawIfNeeded: function() {
        ...
        if (this.activeLayer) {
          if (this.mode == this.MODE.TILESELECT) {
            ...
            this.activeLayer.tileSelect.drawCursor(
              ig.input.mouse.x,
              ig.input.mouse.y,
            );
          }
      ...
      drawLabels: function(step) {
        ...
        for (
          var tx = Math.floor(-this.screen.x % step);
          tx < ig.system.width;
          tx += step
        ) {
          xlabel += step;
        ...
        for (
          var ty = Math.floor(-this.screen.y % step);
          ty < ig.system.height;
          ty += step
        ) {
          ylabel += step;
