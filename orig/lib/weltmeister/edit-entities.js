ig
  .module('weltmeister.edit-entities')
  .requires(
    'impact.game',
  ...
  .defines(function() {
    ...
    wm.EditEntities = ig.Class.extend({
      ...
      init: function(div) {
        ...
        this.div
          .children('.visible')
          .bind('mousedown', this.toggleVisibilityClick.bind(this));
      ...
      importEntityClass: function(modules) {
        ...
        for (var m in modules) {
          ...
          if (className && ig.global[className]) {
            ...
            unloadedClasses.push(
              modules[m] + ' (expected name: ' + className + ')',
            );
          }
        }
      ...
      selectEntityAt: function(x, y) {
        ...
        this.wasSelectedOnScaleBorder = this.isOnScaleBorder(
          ent,
          this.selector,
        );
        return ent;
      ...
      loadEntitySettings: function(ent) {
        ...
        html += this.loadEntitySettingsRecursive(
          this.selectedEntity._wmSettings,
        );
      ...
      showMenu: function(x, y) {
        this.selector.pos = {
          x:
            Math.round((x + ig.editor.screen.x) / this.gridSize) *
            this.gridSize,
          y:
            Math.round((y + ig.editor.screen.y) / this.gridSize) *
            this.gridSize,
        };
        this.menu.css({
      ...
      mousemove: function(x, y) {
        ...
        if (this.selectedEntity) {
          if (
            this.selectedEntity._wmScalable &&
            this.selectedEntity.touches(this.selector)
          ) {
            var scale = this.isOnScaleBorder(
              this.selectedEntity,
              this.selector,
            );
            if (scale == 'n' || scale == 's') {
      ...
      drawEntity: function(ent) {
        ...
        if (ent._wmDrawBox) {
          ig.system.context.fillStyle =
            ent._wmBoxColor || 'rgba(128, 128, 128, 0.9)';
      ...
      drawLineToTarget: function(ent, target) {
        ...
        ig.system.context.lineTo(
          ig.system.getDrawPos(
            target.pos.x + target.size.x / 2 - ig.game.screen.x,
          ),
          ig.system.getDrawPos(
            target.pos.y + target.size.y / 2 - ig.game.screen.y,
          ),
        );
      ...
      drawCursor: function(x, y) {
        if (this.selectedEntity) {
          ...
          ig.system.context.strokeRect(
            ig.system.getDrawPos(
              this.selectedEntity.pos.x - ig.editor.screen.x,
            ) - 0.5,
            ig.system.getDrawPos(
              this.selectedEntity.pos.y - ig.editor.screen.y,
            ) - 0.5,
